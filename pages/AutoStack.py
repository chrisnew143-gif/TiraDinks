    ...
    for i, cid in enumerate(st.session_state.courts):
        with cols[i % 2]:
            ...
            if flat and queue_list:
                out_player = st.selectbox("Player OUT", [p[0] for p in flat], key=f"swap_out_{cid}")
                in_player = st.selectbox("Player IN", [p[0] for p in queue_list], key=f"swap_in_{cid}")
